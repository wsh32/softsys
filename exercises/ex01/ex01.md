...
> When optimization is turned on, the `x` variable disappears. This makes sense, as it is no longer relevant to the program.
...
> When `x` is printed, it shows up regardless of optimization. This is because it is needed at runtime.
...
> No matter the optimization, `x` shows up as a global. This is because there is an operation being done to `x`. In more aggressive optimizations, I expect `x` to go away, as `y` is not used anywhere other than the declaration. 

What conclusions can you draw about how optimization works?

> Optimization strips out unused variables and processes on those variables to
> save as much memory usage and run time. 
